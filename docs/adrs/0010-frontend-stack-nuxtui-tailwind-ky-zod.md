...
- [x] Icon mapping at `web/docs/icons-mapping.md`
- [x] Bundle baseline + delta measured during Slice 1 (benchmark doc removed post-migration)
...
- [x] **Migration closed (spec 073, branch `073-frontend-cleanup`)**: removed the last dangling dependency `@ant-design/icons-vue` (no source imports remained) and the dev-only `/_dev/nuxtui-demo` view/route/spec (its companion EE-gating example spec went with it). Added a permanent `no-restricted-imports` guard in `web/eslint.config.ts` forbidding `ant-design-vue`, `@ant-design/*`, and `axios` (runs in `make lint` / `ci-local`). Authored `web/src/components/ui/README.md`; refreshed the stale `CLAUDE.md` Frontend section. `package.json` now carries zero Ant Design / Axios dependencies. Bundle net-neutral (measured at spec-073 close-out). The AntDV→NuxtUI / Axios→Ky migration (specs 057–073) is complete.
