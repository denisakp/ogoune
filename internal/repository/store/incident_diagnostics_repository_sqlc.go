...
func (r *IncidentDiagnosticsRepositorySQLC) Create(ctx context.Context, d *domain.IncidentDiagnostics) (*domain.IncidentDiagnostics, error) {
	...
	case r.pgQ != nil:
		row, err := r.pgQ.CreateIncidentDiagnostics(ctx, pgsqlc.CreateIncidentDiagnosticsParams{
			...
			DbConnectionsActive:     pgInt8FromPtr(d.DbConnectionsActive),
			DbConnectionsMax:        pgInt8FromPtr(d.DbConnectionsMax),
			DbLongestQuerySeconds:   pgFloat8FromPtr(d.DbLongestQuerySeconds),
			DbReplicationLagSeconds: pgFloat8FromPtr(d.DbReplicationLagSeconds),
		})
		if err != nil {
	...
	case r.sqliteQ != nil:
		row, err := r.sqliteQ.CreateIncidentDiagnostics(ctx, sqlitesqlc.CreateIncidentDiagnosticsParams{
			...
			RootCauseHint:           d.RootCauseHint,
			DbConnectionsActive:     nullInt64FromPtr(d.DbConnectionsActive),
			DbConnectionsMax:        nullInt64FromPtr(d.DbConnectionsMax),
			DbLongestQuerySeconds:   nullFloatFromPtr(d.DbLongestQuerySeconds),
			DbReplicationLagSeconds: nullFloatFromPtr(d.DbReplicationLagSeconds),
		})
		if err != nil {
			return nil, fmt.Errorf("sqlc: create incident diagnostics: %w", err)
		}
...
func incidentDiagnosticsFromPG(row pgsqlc.IncidentDiagnostic) (*domain.IncidentDiagnostics, error) {
	...
	out := &domain.IncidentDiagnostics{
		...
		DbConnectionsActive:     ptrInt64FromPGInt8(row.DbConnectionsActive),
		DbConnectionsMax:        ptrInt64FromPGInt8(row.DbConnectionsMax),
		DbLongestQuerySeconds:   ptrFloatFromPGFloat8(row.DbLongestQuerySeconds),
		DbReplicationLagSeconds: ptrFloatFromPGFloat8(row.DbReplicationLagSeconds),
	}
...
func incidentDiagnosticsFromSQLite(row sqlitesqlc.IncidentDiagnostic) (*domain.IncidentDiagnostics, error) {
	...
	out := &domain.IncidentDiagnostics{
		...
		RootCauseHint:           row.RootCauseHint,
		DbConnectionsActive:     ptrInt64FromNullInt64(row.DbConnectionsActive),
		DbConnectionsMax:        ptrInt64FromNullInt64(row.DbConnectionsMax),
		DbLongestQuerySeconds:   ptrFloatFromNullFloat(row.DbLongestQuerySeconds),
		DbReplicationLagSeconds: ptrFloatFromNullFloat(row.DbReplicationLagSeconds),
	}
	if row.Keyword.Valid {
		s := row.Keyword.String
		out.Keyword = &s
