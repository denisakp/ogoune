...
<template>
  <div class="bg-white rounded-lg border border-slate-200 p-5 mb-5">
    <div class="flex items-start justify-between gap-4 flex-wrap mb-4">
      <div class="flex items-center gap-2.5 flex-wrap">
        ...
        <span class="text-xs font-mono text-slate-500">{{ incidentLabel }}</span>
      </div>
    ...
    <dl class="flex flex-wrap items-center gap-x-6 gap-y-1.5 text-xs">
      <div>
        <dt class="inline text-slate-500">Started:</dt>
        <dd class="inline ml-1 text-slate-700 font-mono">{{ formatDate(incident.started_at) }}</dd>
      </div>
      <div v-if="isResolved">
        <dt class="inline text-slate-500">Resolved:</dt>
        <dd class="inline ml-1 text-slate-700 font-mono">{{ formatDate(incident.resolved_at) }}</dd>
      </div>
    </dl>
  </div>
</template>
